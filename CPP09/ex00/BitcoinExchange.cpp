...
// This extra leap day occurs in each year that is a multiple of 4, except for years
...
// Converts the string to int/double and returns it
...
// Gets the string converted to int and converts it back to a string, then returns it
...
bool BitcoinExchange::fileExists(const char *filename)
{
...
bool BitcoinExchange::fileEmpty(const char *filename)
{
...
bool BitcoinExchange::onlydigits(const string& str, int (*isDigit)(int)) {
...
// Check the validity of the input to handle errors
...
bool BitcoinExchange::validDate(const string &date)
{
    ...
    if ((!onlydigits(year, &::isdigit)) ||
        (!onlydigits(month, &::isdigit)) ||
        (!onlydigits(day, &::isdigit)))
        return false;
...
// Find the accurate previous/next date

string BitcoinExchange::prevDate(const string& date) {
...
void BitcoinExchange::MainFrame(const char *filename)
{
    std::ifstream   file(filename);
    string          line;
